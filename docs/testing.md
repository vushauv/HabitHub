...
This runs frontend, backend unit, and backend integration tests sequentially and exits with a non-zero code if any suite fails. 
...
```zsh
run-tests() { <path-to-the-project-root>/scripts/run_tests.sh "$@" }
